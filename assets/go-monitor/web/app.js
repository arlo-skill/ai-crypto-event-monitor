...
const isHolding = p => p.position_status === 'holding' || num(p.quantity) > 0 || num(p.allocation_fraction) > 0;
const positionBadge = p => isHolding(p) ? badge('持仓中','good') : num(p.allocation_fraction) === 0 ? badge('未建仓','neutral') : badge('仓位待确认','waiting');
...
function holdings(p) {
  if(!p) {
    setHTML('summary','<div class="loading-card">持仓记录暂不可用</div>');
    for(const id of ['positions','orders','allocation','updates','missing']) setHTML(id,'');
    return;
  }
  ...
  const remainingFraction=o=>num(o.status==='open'?o.planned_allocation_fraction:o.remaining_allocation_fraction);
  const unresolved=active.filter(o=>o.status==='partially_filled'&&o.remaining_order_status!=='open');
  const heldKnown=p.allocation_basis==='total_crypto_account'&&positions.length>0&&positions.every(x=>num(x.allocation_fraction)!==null);
  const pendingKnown=unresolved.length===0&&active.every(x=>remainingFraction(x)!==null);
  const held=positions.reduce((sum,x)=>sum+(num(x.allocation_fraction)||0),0), pending=active.reduce((sum,x)=>sum+(remainingFraction(x)||0),0);
  const known=heldKnown&&pendingKnown, left=known?Math.max(0,1-held-pending):null;
  const stats=[
    ['已建仓资产',`${positions.filter(isHolding).length} 种`,heldKnown?`占整个账户 ${percent(held)}`:'持仓已确认，账户总占比待补充'],
    ['待成交买单',unresolved.length?'待核对':`${active.length} 笔`,unresolved.length?`${unresolved.length} 笔订单的剩余状态待确认`:active.length?`剩余计划 ${fraction(pendingKnown?pending:null)} · 与持仓分开`:'记录中无待成交买单'],
    ['未分配计划比例',fraction(left),known&&held+pending>1?'现有持仓与挂单计划已超过账户预算':'按申报比例估算，非实时可用余额']
  ];
  ...
  setHTML('allocation',known?`<div class="allocation"><div class="allocation-bar" role="img" aria-label="已确认仓位 ${esc(fraction(held))}，挂单计划 ${esc(fraction(pending))}"><div class="allocation-held" data-width="${Math.min(100,Math.max(0,held*100))}"></div><div class="allocation-pending" data-width="${Math.min(100,Math.max(0,pending*100))}"></div></div><div class="allocation-labels"><span>● 已确认持仓 ${esc(fraction(held))}</span><span>▨ 未成交计划 ${esc(fraction(pending))}</span></div></div>`:`<div class="allocation"><div class="allocation-labels"><span>● 持仓比例 ${esc(heldKnown?fraction(held):'待确认')}</span><span>▨ 剩余挂单 ${esc(pendingKnown?fraction(pending):'待核对')}</span></div><p class="muted">持仓已按成交记录展示；资金比例资料不足时，不合计原订单与已成交部分。</p></div>`);
  $('allocation').querySelectorAll('[data-width]').forEach(el=>{el.style.width=el.dataset.width+'%';});
  setHTML('positions',positions.map(p=>`<tr><td><span class="position-name">${esc(p.asset)}</span>${positionBadge(p)}<small>${esc(p.symbol)} · 现货</small></td><td>${num(p.quantity)!==null?`${esc(Number(p.quantity).toLocaleString('en-US',{maximumFractionDigits:8}))} 枚`:'数量待确认'}<small>${num(p.reported_asset_value)!==null?`用户报告估值 ${esc(p.reported_asset_value)} ${esc(p.reported_asset_value_currency)}`:''}</small></td><td>${esc(fraction(p.allocation_fraction))}<small>账户 ${esc(percent(p.allocation_fraction))}</small></td><td>${p.average_cost?`${esc(price(p.average_cost))} ${esc(p.cost_currency==='CNY'?'人民币':p.cost_currency)}`:'—'}<small>${p.average_cost?'已确认持仓均价':isHolding(p)?'成交均价待补充':num(p.allocation_fraction)===0?'尚无成交成本':'成交均价待补充'}</small></td><td>${esc(fmtTime(p.confirmed_at))}<small>${esc(p.note)}</small></td></tr>`).join('')||'<tr><td colspan="5">还没有用户确认的仓位记录。</td></tr>');
  setHTML('orders',orders.map(o=>{
    const closed=['filled','cancelled','canceled'].includes(o.status);
    const label=o.status==='partially_filled'&&o.remaining_order_status!=='open'?'部分成交 · 余单待核对':statusLabel[o.status]||o.status;
    return `<tr><td><span class="position-name">${esc(o.asset)}</span>${badge(label,closed?'neutral':'waiting')}<small>${closed?'历史订单 · ':''}${o.side==='buy'?'限价买单':'限价卖单'} · ${esc(o.symbol)}</small></td><td>${esc(fraction(o.planned_allocation_fraction))}<small>${o.status==='open'?'订单计划':o.status==='partially_filled'?'原订单总计划（含已成交）':'原订单计划（历史）'} · ${esc(percent(o.planned_allocation_fraction))}</small></td><td>${esc(price(o.limit_price))} ${esc(o.quote_currency)}<small>用户报告的订单价格</small></td><td>${esc(fmtTime(o.confirmed_at))}<small>${esc(o.note)}</small></td></tr>`;
  }).join('')||'<tr><td colspan="4">暂无订单记录。</td></tr>');
