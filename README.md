...
- **建仓、加仓与减仓信号**：按最新用户持仓分别评估三个方向；有依据时给出条件式价格、仓位变化、操作后敞口及失效条件，资料不足时保留风险提醒，不编造仓位。
...
> 使用 $ai-crypto-event-monitor 核验事件现在是否仍成立，再检查市场、链上、资金流和新闻热度。读取最新持仓台账，分别给出建仓、加仓、减仓判断；有条件式机会时说明价格、增减比例、操作后仓位及失效条件。区分事实与推断，给出来源和数据缺口；仅做研究。
